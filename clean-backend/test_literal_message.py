"""
Test Message model with Literal from typing_extensions
"""
...
class Message(SQLModel, table=True):
    ...
    role: Literal["user", "assistant"] = Field(description="Message sender role")  # Using Literal from typing_extensions
    timestamp: datetime = Field(default_factory=datetime.utcnow, index=True)
    sources: Optional[str] = Field(default=None, description="JSON string of source citations")


if __name__ == "__main__":
    print("Testing message class with Literal from typing_extensions...")
    try:
